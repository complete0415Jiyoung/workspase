...
public class Pagination {
	...
	private int pageSize= 10; 		// 목록하단 페이지번호 노출개수 
	
	private int maxPage; 			// 제일 큰 페이지 번호 == 마지막 페이지 번호 
	private int startPage;			// 목록 하단에 노출된 페이지의 시작 번호
	private int endPage;			// 목록하단에 노출된 페이지의 끝 번호 
	
	private int prevPage;			// 목록하단에 노출된 번호의 이전 목록 끝 번호
	private int nextPage;			// 목록하단에 노출된 번호의 다음 목록 시작 번호
	...
	private void calculatePagination() {
		...
		//전체게시글의 수 : 500// 보여지는 게시글의 수 : 10개
		// -> 미지막 페이지 번호는? 500 / 10 = 50
		
		//전체게시글의 수 : 501// 보여지는 게시글의 수 : 10개
		// -> 미지막 페이지 번호는? 501 / 10 = 51 (50.1 올림처리)

		maxPage = (int)(Math.ceil((double)listCount / limit));
		
		// * startPage : 목록하단에 노출된 페이지의 시작 번호 
