...
// 암호화를 적용해야하는 요청 : 로그인, 회원가입, 회원탈퇴, 비밀번호 변경
// 필터가 적용된 url이 여러개인 경우 : string 배열[]초기화 형태{}로 작성 가능
...
public class EncryptFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// doFilter 메소드 메개변수는 부모타입인 ServletRequest
		// -> 다운 케스팅 필요
		
		HttpServletRequest req= (HttpServletRequest)request;
		
		// 파라미터를 다시 세팅하는 방법은 필터에서 불가능
		// -> Servlet에 Wrapper 클래스를 이용해서 
		//	HttpServletReqest의 메소드를 오버라이딩 할 수 있다
		//	-> 오버라이딩(재정의)를 통해서 비밀번호 암호화 진행
		
		// req을 Wrapper가 감싼 상태,,,,,
		// wrapper 객체를 생성해서 기존 HttpServletReqest 객체의 역할을 대체함
