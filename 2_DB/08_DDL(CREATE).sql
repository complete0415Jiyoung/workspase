...
데이터 딕션너리는 사용자가 테이블을 생성하거나 사용자를 변경하는 등의 
작업을 할때 데이터 베이스 서버에 의해 자동으로 갱신되는 케이블 

--USE_TABLES : 자신의 계정이 소유한 객체 등에 관한 정보를 조회 할 수 있는  
/
--------------------------------------------------------------------------------------------------------------------
...
--1. 테이블 생성하기 
--테이블이란 
--행(ROW)과 열(COLUMN)으로 구분되는 가장 기본적인 데이터 베이스 객체
...
--MEMBER테이블 생성
CREATE TABLE MEMBER(
    MEMBER_ID VARCHAR2(20), --가변길이 문자열 20바이트(영어, 숫자만 작성 시 20글자)
    MEMBER_PWD VARCHAR2(20), 
    MEMBER_NAME VARCHAR2(30), --한글 3바이트 *10 글자 ==30BYTE
    MEMBER_SSN CHAR(14), --970415-2222222
    ENROLL_DATE DATE DEFAULT SYSDATE
);
--Table MEMBER이(가) 생성되었습니다.
--DEFAULT : 입력되는 값이 없거나 'DEFAULT' 키워드 사용시 기록 되어질 값을 지정

--만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; --딕션너리 뷰(데이터 딕션너리에서 일부 만을 뽑아서 만든 가상 테이블)

--접속창 -> 계정 열고 -> 테이블클릭 -> 위쪽 새로고침 클릭 -> MEMBER 테이블 보임
...
--USER_TABLES  : 사용자가 작성한 테이블을 확인하는 딕션너리 뷰
--데이터 테이블 딕션너리 정의되어 있음 
SELECT * FROM USER_TABLES;


--MEMBER 테이블에 샘플 데이터 삽입
INSERT INTO MEMBER VALUES ('MEM01', '123ABC', '홍길동','990808_1234567', DEFAULT);
...
UPDATE MEMBER2 SET MEMBER_SSN = 9908081234567;
SELECT * FROM MEMBER2;

INSERT INTO MEMBER2 VALUES('김영희', 0011134567890); 
-- NUMBER타입 컬럼에 데이터 삽입시 앞자리 0 이 사라짐

SELECT * FROM MEMBER2;

DROP TABLE MEMBER2; --테이블삭제
--------------------------------------------------------------------
--제약 조건(CONSTRAINTS)
/*
...
-- 제약조건 확인
--  USER_CONSTRAINTS : 사용자가 작성한 제약조건을 확인하는 딕션너리 뷰 
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL, -- 사용자 번호 (무조건 있어야함 == NULL X)
                             --> 컬럼레벨 제약조건 설정
    USER_ID VARCHAR2 (20),
...
--ORA-01400: NULL을 ("LJY"."USER_USED_NN"."USER_N0") 안에 삽입할 수 없습니다
...
-- 컬럼레벨, 테이블 레벨에서 설정 가능
-- 단, UNIQUE 제약 조건이 설정된 컬럼에 NULL 값 (값이 없는 상태)은 중복 삽입 가능.


-- UNIQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
--  USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약조건 작성법 (제약조건명 미지정)
...
INSERT INTO USER_USED_UK
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> 같은 아이디인 데이터가 이미 테이블에 있으므로 UNIQUE 제약 조건 위배되어 오류 발생 
-- ORA-00001: 무결성 제약 조건(LJY.UER_ID_U)에 위배됩니다
                --UNIQUE 
...
-- 제약조건 2-1. UNIQUE 복합키 
-- 두개 인상의 컬럼을 묶어서 하나의 UNIQUE 제약조건을 설정함
--> 모든 컬럼의 값이 동일해야 중복으로 취급함 
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(LJY.USER_ID_NAME_U)에 위배됩니다 
--> 어러 컬럼을 묶어서 UINQUE 제약조건이 설정 되어 있으면 
-- 두 컬럼이 모두 중복되는 값일 경우에만 오류 발생

SELECT * FROM USER_USED_UK2;
--------------------------------------------------------------------------
--제약조건 3. PRIMARY KEY(기본키)제약조건

-- 케이블에서 한 행의 정보를 찾기 위해 사용할 컬럼을 의미함 
-- 테이블에 대한 식별자(IDENTIFIER)역할를 함 
-- NOLL NULL + UNOQUE 제약조건을 의미 
-- 한 테이블 당 한 개만 설정할 수 있음 
-- 컬럼레벨, 테이블레벨 둘다 설정 가능 
-- 한개 컬럼에 설정할 수 있고, 여러개 컬럼을 묶어서 설정할 수 있음(복합키)
...
--> 기본키 중복으로 오류 
--ORA-00001: 무결성 제약 조건(LJY.USER_NO_PK)에 위배됩니다
INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가  NILL 이므로  오류
--ORA-01400: NULL을 ("LJY"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다.
-----------------------------------------------------------------------
--제약조건 3-1. PRIMARY KEY 복합키 제약조건(테이블레벨만 가능)
...
--ORA-00001: 무결성 제약 조건(LJY.PK_USERNO_USERID)에 위배됩니다
--회원 번호와 아이디 둘다 중복 되었을 때 만  제약 조건 위배 발생 

SELECT * FROM USER_USED_PK2;

--PRIMARY KEY 는 NULL이 들어갈 수 앖음
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
--ORA-01400: NULL을 ("LJY"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다.
-->복합키여도 PK이므로 NULL절대 안됨

----------------------------------------------------------------------------------

-- 제약조건 4. FOREIGN KEY(외부키/외래키) 
...
--> 50이라는 값은 USER_GRADE 테이블에 GRADE_CODE 컬럼에 제공하는 값이 아니므로
--   외래키 제약조건 위배되어 오류 발생
--ORA-02291: 무결성 제약조건(LJY.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
            --> 참조 무결성 위배

--------------------------------------------------------------------------------

--* FOREIGN KEY 삭제 옵션
--부모 테이블의 데이터를 삭제시 자신테이블의 데이터를 
...
WHERE GRADE_CODE =10; 
--ORA-02292: 무결성 제약조건(LJY.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다


-- 1) ON DELETE RESTRICTED (삭제 제한)로 기본 지정 되어 있음
-- FOREIGN KEY로 지정된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함

--GRADE CODE 중에서 20은 외래키를 참조 되고 있지 않으므로 삭제 가능
COMMIT;
...
--부모테이블이 USER_GRADE 에서 GRADE_CODE =10 삭제
DELETE FROM USER_GRADE3
WHERE GRADE_CODE =10;

SELECT* FROM USER_GRADE3; --부모행 삭제확인 (10 없어짐)
SELECT* FROM USER_USED_FK3;--10을 참조하는 자식행 삭제 확인 (종속 관계의 행이 모두 삭제됨)
--ON DELETE CASCADE 옵션으로 인한 참조키를 사용한 행이 삭제됨을 확인

-------------------------------------------------------------------------

--제약조건 5. CHECK 제약조건 : 컬럼에 기록되는 값에 조건을 설정할 수 있음 
--CHECK (컬럼명 비교연산자 비교값) 
--주의 : 비교값은 리터럴만 사용할 수 있음, 변하는 값이나 힘수 사용 못함
...
--ORA-02290: 체크 제약조건(LJY.GENDER_CHECK)이 위배되었습니다
--GENDER컬럼에 CHECK 제약조건으로 '남','여'만 기록이 가능한데
--'남자'라는 조건 이외의 값이 들어 와서 에러 발생 

--CHECK 제약조건은 범위로도 설정가능 
--------------------------------------------------------------------
...
VALUES (1, 'user1', 'pass01', '880122-1234567', '남', '010-1111-9999','서울시 강남구 역삼동','N');
INSERT INTO USER_TEST
...
-- 컬럼명, 데이터타입, 값이 복사되고 , 제약 조건은 NOT NULL만 복사 
...
--3) 테이블 형태 (컬럼명, 데이터타입)만 복사 (데이터 복사 X) 
CREATE TABLE EMPLOYEE_COPY3 
AS SELECT * FROM EMPLOYEE
    WHERE 1 = 0; --거짓인 테이블 만들어서  
...
-- NOT NULL 제약 조건만 EMPLOYEE테이블에 복사됨 
